...
- **explicit questionnaire phase acknowledgements** for blind request receipt, credential receipt, and submission receipt, so resend logic can stop once delivery is confirmed instead of inferring success only from later state; a valid credential acknowledgement also advances the organiser's participant state to **Ballot received**, including when a delegated proxy issued the credential

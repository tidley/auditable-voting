...
3. Add or import voter `npub`s in **Voters** when you want to invite voters once and reuse that eligibility for later questionnaires from the same organiser. Each invited voter can have an internal note and an **Auto-ballot** checkbox. Click **Apply to current questionnaire** to project checked voters into the active questionnaire whitelist and publish one roster-free public questionnaire announcement. After one questionnaire is published, **Add session** appears under **Questionnaire**, creates a fresh questionnaire ID from the current setup, and offers only **Publish to invited voters**, which publishes and projects checked voters in one flow. The roster is organiser-local; it is not published and is not a reusable ballot credential.
...
3. Click **Request ballot** if the blind credential request does not start automatically. Current per-question questionnaires request a bundle with one scoped credential per ballot index; large request bundles may be compressed before NIP-17 encryption, with plain JSON kept as the fallback.
...
- voter questionnaire submissions now spend blind-signed credentials from a fresh ephemeral response npub; current per-question questionnaires request one credential per ballot index and spend one scoped credential with each submitted ballot group

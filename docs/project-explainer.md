...
Current live evidence is mixed: `1 coordinator / 2 voters / 2 rounds` and single-coordinator runs up to `1 coordinator / 20 voters / 3 rounds` have completed cleanly in recent local-preview tests, and the live harness now waits for the lead coordinator to be visibly ready before firing round 1, which removed the earlier blind first-round startup miss. But `2 coordinators / 2 voters / 2 rounds` is still not reliable enough across reruns to call signed off because later-round ticket/ack behaviour can still degrade, larger public-relay runs such as `5 coordinators / 10 voters / 3 rounds` are not yet signed off, and `5 coordinators / 10 voters / 10 rounds` is still not operationally viable.
...
The current app now does better at small scale by limiting live read fanout, widening coordinator-control and ticket/ack traffic slightly beyond ordinary DM reads, backfilling both live rounds and ticket traffic from history, gating the first multi-coordinator round on MLS welcome acknowledgement after non-lead control-plane sync, waiting in the live harness until the lead is visibly ready before firing round 1, queueing DM publishes per sender-recipient conversation instead of per recipient only, and preserving prior ticket attempt identifiers so later acknowledgements are not lost when a resend replaces the latest tracked response. That is enough to make the failure mode much clearer, but it is still not enough to make large committee runs production-grade on public relays: the latest trustworthy `5 / 10 / 3` run now gets materially through round 1, then times out later under a mixed send/ack bottleneck, with acknowledgement visibility still worse than send-side delivery.

...
Before publishing, the organiser makes a bounded query of public relays for an existing definition with the same questionnaire ID. A different definition blocks publication and requires regenerating the ID, while an identical definition signed by the same organiser is recognised as an already-published retry. Observer presents same-ID definitions as distinct variants with organiser identity, definition event ID, and time; `q`, `coordinator`/`organiser`, and `definition` URL parameters can pin the intended variant, and proof verification uses that selected variant or an explicit response definition reference.

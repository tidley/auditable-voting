...
- **NIP-17 gift-wrapped DMs** for follow, issuance, direct ticket delivery, acknowledgement traffic, and history-based recovery; the current client now keeps the shard-request DM id aligned with the blind-request id so retries and acknowledgement recovery treat one logical request consistently
...
Current live evidence is mixed: `1 coordinator / 2 voters / 2 rounds` and single-coordinator runs up to `1 coordinator / 20 voters / 3 rounds` have completed cleanly in recent local-preview tests, and the live harness now waits for the lead coordinator to be visibly ready before firing round 1, which removed the earlier blind first-round startup miss. A fresh `v0.73` rerun of `2 coordinators / 2 voters / 2 rounds` also completed cleanly after normalising shard-request ids across the request/ticket/ack path, and repeated `2 / 2 / 2` runs improved after that repair. In `v0.74`, coordinator pages also expose explicit runtime-readiness phases, and failed live harness runs now carry protocol-layer classification plus coordinator/voter readiness snapshots. But that small multi-coordinator case is still not signed off as repeatedly reliable across reruns, larger public-relay runs such as `5 coordinators / 10 voters / 3 rounds` are not yet signed off, and `5 coordinators / 10 voters / 10 rounds` is still not operationally viable.

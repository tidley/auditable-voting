...
4. Share invite links from **Voting**. The General QR/link opens **Vote** directly and requests a ballot automatically; single-use private links are created and labelled in **Admitted voters**. For already admitted voters, **Apply to current questionnaire** green-lights rows with **Auto-ballot** ticked and publishes one public questionnaire announcement that their Vote page can discover.

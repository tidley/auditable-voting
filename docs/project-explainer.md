...
- The audit proxy still follows organiser eligibility: it waits for the organiser's whitelist/private-code config before issuing, preserves per-question ballot scope in issued credentials, and general-link requests are also copied to the organiser so approval can update the proxy without a second voter request. It only scans public submissions for the newest configured questionnaire with a positive expected participant count, and stops scanning once expected accepted completion is reached. It does not create follow-up questionnaires; that remains an organiser action because it depends on the private admitted-voter roster.

...
- the lead only publishes the public live round after that control state reaches round-open agreement
...
Current live evidence is mixed: `1 coordinator / 2 voters / 2 rounds`, `2 coordinators / 2 voters / 2 rounds`, and single-coordinator runs up to `1 coordinator / 20 voters / 3 rounds` have completed cleanly in recent local-preview tests, but larger public-relay runs such as `5 coordinators / 10 voters / 3 rounds` are not yet signed off and `5 coordinators / 10 voters / 10 rounds` is still not operationally viable.

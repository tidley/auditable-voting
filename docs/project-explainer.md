...
2. The voter requests fresh blind ballot credentials for this questionnaire. New questionnaires send one bundled private DM with one scoped blinded message for each current ballot index, or two per ballot index when that voter is marked as a proxy voter; large bundles can be compressed before encryption, while small bundles stay plain JSON.
...
4. The voter submits each ballot index as a public anonymous ballot; grouped questions share one proof.
...
- The client uses tag-filtered, paginated public reads, adaptive NIP-17 mailbox recovery, local-key live request-ack/credential subscriptions, and consolidated organiser, voter, and observer live questionnaire subscriptions, but relay rate limits can still affect busy demonstrations.
...
- Final result packs can be mirrored to Blossom as public gzip blobs, but Observer verifies the compressed blob size and SHA-256 before showing rows from that fallback.
...
- If the organiser changes an answer-bearing question after credentials exist, the app bumps that ballot index group's slot version and voters need a fresh scoped credential for that slot.

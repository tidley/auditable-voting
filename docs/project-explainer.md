...
2. The voter requests fresh blind ballot credentials for this questionnaire. New questionnaires send one bundled private DM with one scoped blinded message for each current ballot index; large bundles can be compressed before encryption, while small bundles stay plain JSON.

...
- **seven-day fixed-lookback audit proxy DM polling with event-id dedupe**, so relay-randomised gift-wrap timestamps do not cause missed delegated blind requests; the helper publishes status heartbeats back to the coordinator, uses active delegation control relays in addition to startup relays, can publish a delegated close event plus result summary once all expected invitees have accepted valid responses, and defaults to control relays that accept `#p` gift-wrap reads

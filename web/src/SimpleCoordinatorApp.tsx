...
import {
  questionnaireResponseDetailMatchesSearch,
...
function setPrivateInviteUrlAutoBallot(inviteUrl: string, autoRequestBallot: boolean) {
  ...
  try {
    ...
    if (autoRequestBallot) {
      url.searchParams.set("request_ballot", "1");
    } else {
      url.searchParams.delete("request_ballot");
      url.searchParams.delete("auto_request");
    }
    return url.toString();
  } catch {
    return inviteUrl;
...
function setPrivateInviteUrlBallotGroup(inviteUrl: string, ballotGroup?: string | null) {
  if (!inviteUrl.trim()) {
    return "";
  }
  try {
    ...
    if (normalised) {
      url.searchParams.set("ballot_group", normalised);
    } else {
      url.searchParams.delete("ballot_group");
      url.searchParams.delete("ballotGroup");
    }
    return url.toString();
  } catch {
    return inviteUrl;
...
type InviteActionIconName = "contacts" | "invite" | "copy" | "single-use";

function InviteActionIcon({ name }: { name: InviteActionIconName }) {
  const paths: Record<InviteActionIconName, string[]> = {
    contacts: [
      "M8 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z",
      "M2.75 19c.45-2.75 2.3-4.5 5.25-4.5s4.8 1.75 5.25 4.5H2.75Z",
      "M16.5 7.5h4",
      "M18.5 5.5v4",
      "M16.2 14.5c2.2.25 3.7 1.7 4.05 4.5h-4.1",
    ],
    invite: [
      "M4 6.5h10.5a3.5 3.5 0 0 1 0 7H8.25",
      "M8.25 13.5 11 10.75",
      "M8.25 13.5 11 16.25",
      "M17.5 5.5v4",
      "M15.5 7.5h4",
    ...
    "single-use": [
      "M7 12.5 10 15.5 17 8.5",
      "M12 2.75 4.75 5.5v6.1c0 4.6 3.05 7.85 7.25 9.65 4.2-1.8 7.25-5.05 7.25-9.65V5.5L12 2.75Z",
    ],
  };

  return (
    <svg
      className='simple-button-inline-icon'
      viewBox='0 0 24 24'
      aria-hidden='true'
      focusable='false'
    >
...
export default function SimpleCoordinatorApp({ accountMenu }: SimpleCoordinatorAppProps = {}) {
  ...
  const updateQuestionnaireRosterAnnouncement = useCallback((nextStatus: {
    ...
    setQuestionnaireRosterAnnouncement((current) => {
      ...
      const questionnaireId = nextQuestionnaireId || current.questionnaireId;
      const state = nextStatus.state ?? current.state;
      if (current.questionnaireId === questionnaireId && current.state === state) {
        return current;
      }
      return {
  ...
  const handleCoordinatorResponseDetailsChange = useCallback((responseDetails: QuestionnaireResultsDashboardResponseDetail[]) => {
    setCoordinatorParticipantResponseDetails(responseDetails);
  }, []);
  ...
  const optionAParticipantSubmissionEntries = useMemo(() => {
    ...
    const responseDetailBySubmissionId = new Map(
      coordinatorParticipantResponseDetails.map((entry) => [entry.response.responseId, entry]),
    );
    return Object.values(snapshot.receivedSubmissions)
      .filter((submission) => submission.electionId === snapshot.election.electionId)
      .map((submission) => {
        const invitedNpub =
          submission.responseNpub?.trim()
          || submission.invitedNpub?.trim()
          || submission.submissionId;
        return {
          submission,
          acceptance: snapshot.acceptanceResults[submission.submissionId] ?? null,
          responseDetail: responseDetailBySubmissionId.get(submission.submissionId) ?? null,
          invitedNpub,
        };
      })
      .sort((left, right) => {
        const leftSubmittedAt = Date.parse(left.submission.submittedAt);
        const rightSubmittedAt = Date.parse(right.submission.submittedAt);
        return (Number.isFinite(rightSubmittedAt) ? rightSubmittedAt : 0) - (Number.isFinite(leftSubmittedAt) ? leftSubmittedAt : 0);
      });
  }, [coordinatorParticipantResponseDetails, knownVoterInviteRefreshNonce, optionACoordinatorRuntime]);
  const admittedVoterDisplayRows = useMemo(() => {
    const admittedEntryByNpub = new Map(
      admittedVoterEntries
        .map((entry) => [entry.npub.trim(), entry] as const)
    ...
    const byNpub = new Map<string, {
      ...
      privateInviteEntry: BearerInviteCodeEntry | null;
      submissionEntry: (typeof optionAParticipantSubmissionEntries)[number] | null;
    ...
    const chooseSubmissionEntry = (
      ...
      next: (typeof optionAParticipantSubmissionEntries)[number],
    ) => {
      if (!current) {
        return next;
      }
      if (next.acceptance?.accepted && current.acceptance?.accepted !== true) {
        return next;
      }
      if (current.acceptance?.accepted && next.acceptance?.accepted !== true) {
        return current;
      }
      const currentSubmittedAt = Date.parse(current.submission.submittedAt);
      const nextSubmittedAt = Date.parse(next.submission.submittedAt);
      return (Number.isFinite(nextSubmittedAt) ? nextSubmittedAt : 0) > (Number.isFinite(currentSubmittedAt) ? currentSubmittedAt : 0)
        ? next
        : current;
    };
    ...
    const attachAdmittedEntry = (row: ReturnType<typeof ensureRow>) => {
      if (!row) {
        return;
      }
    ...
    for (const entry of optionAParticipantSubmissionEntries) {
      const row = ensureRow(entry.invitedNpub);
      if (row) {
        attachAdmittedEntry(row);
        row.submissionEntry = chooseSubmissionEntry(row.submissionEntry, entry);
      }
    }
    const knownSubmissionIds = new Set(optionAParticipantSubmissionEntries.map((entry) => entry.submission.submissionId));
    ...
    return [...byNpub.values()].filter((row) => (
      ...
      const priority = (row: typeof left) => {
        ...
        if (row.currentQuestionnaireEntry || row.privateInviteEntry) {
          return 1;
        }
        if (row.submissionEntry || row.responseDetail) {
          return 2;
        }
        return 3;
      };
  ...
  const participantReceivedVoteCount = useMemo(() => {
    ...
    for (const detail of coordinatorParticipantResponseDetails) {
      responseIds.add(detail.response.responseId);
    }
    return responseIds.size;
  }, [coordinatorParticipantResponseDetails, optionAParticipantSubmissionEntries]);
  const admittedVoterSummaryLabel = useMemo(() => {
    ...
    if (participantReceivedVoteCount > 0) {
  ...
  const filteredAdmittedVoterDisplayRows = useMemo(() => {
    ...
    if (!query) {
      return admittedVoterDisplayRows;
    }
    return admittedVoterDisplayRows.filter((row) => {
      ...
      const searchableValues = [
        ...
        row.admittedEntry?.note ?? "",
        row.currentQuestionnaireEntry?.claimState ?? "",
        row.pendingAuthorization ? `wants access ${row.pendingAuthorization.requestCount}` : "",
        ...
        submission?.responseNpub ? deriveIdentityWords(submission.responseNpub) : "",
        responseDetail?.response.authorPubkey ? deriveIdentityWords(responseDetail.response.authorPubkey) : "",
        submission?.tokenCommitment ?? "",
        submission?.nullifier ?? "",
        submissionEntry?.acceptance?.reason ?? "",
        submissionEntry?.acceptance?.accepted === true ? "accepted vote accepted" : "",
        submissionEntry?.acceptance?.accepted === false ? "rejected invalid vote rejected" : "",
      ];
      return (
  ...
  const participantTableColumns = useMemo<ColumnDef<ParticipantDisplayRow>[]>(() => {
    const rowState = (participant: ParticipantDisplayRow) => {
      ...
      const statusLabel = pendingAuthorization
        ? `Requested (${pendingAuthorization.requestCount})`
      ...
      const isProxyPrivateInvite = Boolean(
        privateInviteEntry?.credentialsPerVoter === 2
        || (isPrivateInviteClaimant && currentQuestionnaireEntry?.credentialsPerVoter === 2),
      );
      return {
        currentQuestionnaireEntry,
        pendingAuthorization,
    ...
    const nextColumns: ColumnDef<ParticipantDisplayRow>[] = [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const voterLabel = state.isUnclaimedPrivateInvite
            ? state.isProxyPrivateInvite ? "Proxy invite" : "Private invite"
            : state.isResultOnlyParticipant
              ? "Result only"
              : deriveActorDisplayId(participant.npub);
          const fullIdentityLabel = state.isUnclaimedPrivateInvite
            ? state.isProxyPrivateInvite ? "Unclaimed proxy private link" : "Unclaimed private link"
            : participant.npub;
          return (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const participant = row.original;
          const state = rowState(participant);
          const ballotGroupSelectId = state.privateInviteEntry
            ? `admitted-voter-ballot-private-${state.privateInviteEntry.codeHash}`
            : `admitted-voter-ballot-${participant.npub}`;
          return state.canSetBallotGroup ? (
        ...
        cell: ({ row }) => {
          ...
          return state.canToggleProxyVoter ? (
            ...
            <span className={`simple-admitted-voter-proxy-label${state.proxyVoterEnabled ? " is-proxy" : ""}`}>
              {state.proxyVoterEnabled ? "2 votes" : "1 vote"}
            </span>
          ) : (
            <span className='simple-admitted-voter-empty-cell'>-</span>
          );
  ...
  useEffect(() => {
    ...
    try {
      const result = optionACoordinatorRuntime.addWhitelistNpubs(admittedVoterNpubs, {
        credentialsByNpub: credentialsByAdmittedNpub(admittedVoterNpubs),
        ballotGroupsByNpub: ballotGroupsByAdmittedNpub(admittedVoterNpubs),
      });
  ...
  }, [activeCoordinatorNpub, optionACoordinatorRuntime, optionAElectionId, admittedVoterNpubKey, admittedVoterWorkerConfigKey]);
  ...
  useEffect(() => {
    ...
    if (!npub) {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent(SIMPLE_IDENTITY_UPDATED_EVENT, {
          detail: { role: "coordinator", npub: "", nsec: "" },
        }));
      }
      return;
    }
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent(SIMPLE_IDENTITY_UPDATED_EVENT, {
        detail: { role: "coordinator", npub, nsec: keypair?.nsec?.trim() ?? "" },
      }));
    }
  }, [activeCoordinatorNpub, keypair?.nsec]);
  ...
  async function copyPublicQuestionnaireInviteLink() {
    const copied = await tryWriteClipboard(publicQuestionnaireInviteUrl);
    if (copied) {
      showCopyLabel("public-questionnaire-invite");
    }
    setKnownVoterInviteStatus("");
  }

  async function copyPrivateInviteCodeLink(codeHash: string, copiedLabelKey?: string) {
    const inviteUrl = getPrivateInviteCodeLink(codeHash);
    if (!inviteUrl) {
    ...
    if (copied && copiedLabelKey) {
      showCopyLabel(copiedLabelKey);
    }
  ...
  function getPrivateInviteCodeLink(codeHash: string) {
  ...
  function ballotGroupsByAdmittedNpub(npubs: string[], voters = admittedVoters) {
    return Object.fromEntries(
      npubs
        .map((npub) => [npub, normaliseQuestionnaireBallotGroup(voters[npub]?.ballotGroup)] as const)
  ...
  function projectNpubsToQuestionnaire(npubs: string[], questionnaireId = optionAElectionId.trim(), voters = admittedVoters) {
    ...
    try {
      const result = runtimeHandle.runtime.addWhitelistNpubs(npubs, {
        ...
        ballotGroupsByNpub: ballotGroupsByAdmittedNpub(npubs, voters),
      });
      if (result.addedCount > 0 || result.changed) {
        setKnownVoterInviteRefreshNonce((value) => value + 1);
  ...
  function admitVotersToRoster(values: string[], source: AdmittedVoterRecord["source"], options?: { silent?: boolean }) {
    ...
    if (!coordinatorNpub) {
      if (!options?.silent) {
        setAdmittedVoterStatus("Login as organiser before inviting voters.");
      }
      return {
        npubs: [] as string[],
        addedCount: 0,
  ...
  async function inviteDraftVoter() {
    ...
    if (!optionACoordinatorRuntime || !optionAElectionId.trim()) {
      const result = admitVotersToRoster([npub], "manual");
    ...
    setAdmittedVoterDraftNpub("");
    setInviteFeedbackStatus(`Inviting ${deriveActorDisplayId(npub)}...`, "admitted");
    setKnownVoterInviteRefreshNonce((value) => value + 1);
    await sendInviteToKnownVoter(npub, { statusTarget: "admitted" });
  }
  ...
  function removeVoterAdmission(npub: string) {
    ...
    refreshAdmittedVoterRoster(next);
    setAdmittedVoterStatus(`Removed ${deriveActorDisplayId(npub)} from future questionnaire invites. Existing questionnaire state is unchanged.`);
  }

  async function applyAdmissionRosterToCurrentQuestionnaire(options?: {
    questionnaireId?: string;
    suppressUnchangedStatus?: boolean;
  }) {
    ...
    try {
      setAdmittedVoterStatus("Applying invited voters to this questionnaire...");
      ...
      await syncActiveWorkerElectionConfig(questionnaireId).catch(() => false);
      if (addedCount > 0) {
        setAdmittedVoterStatus(
          `Green-lit ${addedCount} invited voter${addedCount === 1 ? "" : "s"} for this questionnaire; ${announcementText}. Voters discover it publicly and request blind ballots when they answer.`,
        );
      } else if (options?.suppressUnchangedStatus) {
        setAdmittedVoterStatus(null);
      } else {
        setAdmittedVoterStatus(`${announcementText}.`);
      }
    } catch (error) {
      setAdmittedVoterStatus(error instanceof Error ? error.message : "Could not apply invited voters.");
    } finally {
  ...
  function buildActiveWorkerElectionConfigSnapshot(questionnaireId = optionAElectionId): { snapshot: WorkerElectionConfigSnapshot; delegation: WorkerDelegationCertificate; workerNpub: string; relays: string[] } | null {
    ...
    const proxyVoterNpubs = [...whitelistByNpub.entries()]
      ...
      .map(([npub]) => npub);
    const ballotGroupsByNpub: Record<string, string> = Object.fromEntries(
      [...whitelistByNpub.entries()]
        .filter(([, entry]) => Boolean(entry.ballotGroup))
        .map(([npub, entry]) => [npub, entry.ballotGroup as string] as const),
    );
    ...
    const workerBearerInviteCodes = bearerInviteCodes.map(({
      note: _note,
      autoRequestBallot: _autoRequestBallot,
      markedUsedAt: _markedUsedAt,
      ...entry
    }) => entry);
  ...
  async function syncActiveWorkerElectionConfig(questionnaireId = optionAElectionId, options?: { force?: boolean; approvedVoterNpub?: string }) {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      if (queue.inFlight || queue.scheduled) {
        return;
      }
      queue.scheduled = true;
      globalThis.setTimeout(() => {
        void flushWorkerConfigSyncQueue();
      }, 0);
    });
  }

  async function flushWorkerConfigSyncQueue() {
    const queue = workerConfigSyncRef.current;
    ...
    try {
      ...
      if (queue.pendingElectionIds.size > 0 && !queue.scheduled) {
        queue.scheduled = true;
        globalThis.setTimeout(() => {
          void flushWorkerConfigSyncQueue();
        }, 0);
  ...
  useEffect(() => {
    const electionId = optionAElectionId.trim();
    if (!activeQuestionnaireWorkerConfigKey || !electionId) {
      return;
    }
  ...
  function buildKnownVoterInviteLink(invitedNpub: string) {
    ...
    return buildQuestionnaireInviteUrl({
      ...
      credentialsPerVoter: whitelistEntry?.credentialsPerVoter === 2
        ...
        ? 2
        : undefined,
      ballotGroup: normaliseQuestionnaireBallotGroup(whitelistEntry?.ballotGroup ?? admittedVoters[normalizedInvitedNpub]?.ballotGroup),
    });
  }
  ...
  async function addSelectedImportedContactsToWhitelist() {
    ...
    if (result.npubs.length === 0) {
      setKnownVoterInviteStatus("Could not add selected contacts.");
      return;
    }
    if (!optionACoordinatorRuntime || !optionAElectionId.trim()) {
      setKnownVoterInviteStatus(
        `Invited ${result.addedCount || result.npubs.length}/${selectedImportedKnownVoterNpubs.length} selected contact${selectedImportedKnownVoterNpubs.length === 1 ? "" : "s"}${optionAElectionId.trim() ? `; ${result.projectedCount} added to this questionnaire.` : "."}`,
      );
      return;
    }
    setKnownVoterInviteStatus(`Inviting ${selectedImportedKnownVoterNpubs.length} selected contact${selectedImportedKnownVoterNpubs.length === 1 ? "" : "s"}...`);
    ...
    setSelectedImportedKnownVoterNpubs([]);
    setKnownVoterInviteStatus(
      `Invited ${selectedImportedKnownVoterNpubs.length} selected contact${selectedImportedKnownVoterNpubs.length === 1 ? "" : "s"}; ${deliveredCount}/${selectedImportedKnownVoterNpubs.length} DM${selectedImportedKnownVoterNpubs.length === 1 ? "" : "s"} sent.`,
    );
  }
  ...
  return (
    <main className={`simple-voter-shell simple-coordinator-shell${sidebarCollapsed ? " is-sidebar-collapsed" : ""}`}>
      ...
      {!sidebarCollapsed ? (
        <button
          type='button'
          className='simple-coordinator-sidebar-backdrop'
          aria-label='Close organiser menu'
          ...
          >
            ...
            <div id='coordinator-invite-voters-section' className='simple-session-invites'>
              <SimpleCollapsibleSection
                ...
                hideToggle
                headerAction={(
                  <span className='simple-admitted-voter-count simple-admitted-voter-count-header'>
                    {admittedVoterSummaryLabel}
                  </span>
                )}
              >
                <div className='simple-invite-share-panel simple-admitted-voters-panel' aria-label='Voters'>
	                  <div className='simple-participants-subsection' aria-label='Participants'>
                  ...
                  ) : participantRosterVisibleCount === 0 ? (
                    <p className='simple-voter-empty'>
          ...
          >
            ...
            >
              ...
              <label className='simple-settings-toggle'>
                ...
                />
                <span>Use relay hints</span>
              </label>
              <p className='simple-voter-note'>
                Advanced. Helps clients find your relays.
              </p>
            </section>

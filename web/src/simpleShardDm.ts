...
export type SimpleForwardedRoundTicket = {
  id: string;
  dmEventId: string;
  requestId: string;
  coordinatorNpub: string;
...
function parseSimpleForwardedRoundTicket(
  wrappedEvent: Record<string, unknown> & { id?: string; created_at: number },
  secretKey: Uint8Array,
): SimpleForwardedRoundTicket | null {
  try {
    const rumor = nip17.unwrapEvent(wrappedEvent as never, secretKey) as {
      content: string;
    };
    ...
    if (
      payload.action !== 'simple_round_ticket_forward' ||
      !payload.response_id ||
      !payload.request_id ||
      !payload.recipient_npub ||
      !payload.threshold_label ||
      !payload.blind_share_response
    ) {
      return null;
...
export async function sendSimpleForwardedRoundTicketToLead(input: {
  coordinatorSecretKey: Uint8Array;
  blindPrivateKey: SimpleBlindPrivateKey;
  keyAnnouncementEvent: any;
  leadCoordinatorNpub: string;
  ...
  const event = nip17.wrapEvent(
    ...
    JSON.stringify({
      action: "simple_round_ticket_forward",
      response_id: responseId,
      request_id: input.request.blindRequest.requestId,
      recipient_npub: input.request.replyNpub,
      coordinator_npub: input.coordinatorNpub,
      threshold_label: input.thresholdLabel,
      voting_prompt: input.votingPrompt,
      blind_share_response: blindShareResponse,
...
export async function sendSimpleForwardedRoundTicketToRecipient(input: {
  forwarderSecretKey: Uint8Array;
  forwarderNpub: string;
  recipientNpub: string;
  forwardedTicket: SimpleForwardedRoundTicket;
  relays?: string[];
}): Promise<DmPublishResult & { responseId: string }> {
  const decoded = nip19.decode(input.recipientNpub);
  if (decoded.type !== "npub") {
  ...
  await publishOwnNip65RelayHints({
    secretKey: input.forwarderSecretKey,
    inboxRelays: dmRelays,
    outboxRelays: dmRelays,
    publishRelays: dmRelays,
    channel: `nip65:${input.forwarderNpub}`,
  }).catch(() => null);
  const event = nip17.wrapEvent(
    input.forwarderSecretKey,
    {
      publicKey: decoded.data as string,
      relayUrl: dmRelays[0],
    },
    JSON.stringify({
      action: "simple_round_ticket",
      response_id: input.forwardedTicket.id,
      request_id: input.forwardedTicket.requestId,
      coordinator_npub: input.forwardedTicket.coordinatorNpub,
      threshold_label: input.forwardedTicket.thresholdLabel,
      voting_prompt: input.forwardedTicket.votingPrompt,
      blind_share_response: input.forwardedTicket.blindShareResponse,
      created_at: input.forwardedTicket.createdAt,
    }),
    "Forwarded round ticket",
  );
...
export async function fetchSimpleForwardedRoundTickets(input: {
  leadCoordinatorNsec: string;
  relays?: string[];
}): Promise<SimpleForwardedRoundTicket[]> {
  const { secretKey, publicHex: leadCoordinatorHex, npub: leadCoordinatorNpub } = getNpubFromNsec(
    input.leadCoordinatorNsec,
    "Lead coordinator",
  );
  ...
  const tickets = new Map<string, SimpleForwardedRoundTicket>();
  for (const wrappedEvent of wrappedEvents) {
    const ticket = parseSimpleForwardedRoundTicket(wrappedEvent, secretKey);
    if (ticket) {
      tickets.set(ticket.id, ticket);
    }
  }

  return sortByCreatedAtDescending([...tickets.values()]);
}

export function subscribeSimpleForwardedRoundTickets(input: {
  leadCoordinatorNsec: string;
  relays?: string[];
  onTickets: (tickets: SimpleForwardedRoundTicket[]) => void;
  onError?: (error: Error) => void;
}): () => void {
  const { secretKey, publicHex: leadCoordinatorHex, npub: leadCoordinatorNpub } = getNpubFromNsec(
    input.leadCoordinatorNsec,
  ...
  void fetchSimpleForwardedRoundTickets({
    leadCoordinatorNsec: input.leadCoordinatorNsec,
    relays: input.relays,
  }).then((initialTickets) => {
    if (closed) {
    ...
    input.onTickets(sortByCreatedAtDescending([...tickets.values()]));
  }).catch((error) => {
    if (!closed && error instanceof Error) {
      input.onError?.(error);
    }

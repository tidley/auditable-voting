...
Auditable Voting lets an organiser publish a questionnaire, invite voters for repeated questionnaire rounds, issue fresh blind ballot credentials, accept public blind-token responses, and let observers verify the public result stream. New questionnaire flows can bundle one question-scoped credential per answered question so a voter can answer a multi-question AGM-style ballot with a single submit action. It runs as a static web app, with an optional outbound-only Rust audit proxy for organiser-offline issuance, verification, closing, and result publication.
...
- Voter and organiser **Messages** sections for normal NIP-17 gift-wrapped direct messages, so voters can ask for help and organisers can reply without exposing the conversation publicly. Enter sends from message windows, Shift+Enter adds a line break, and voter public-link sessions scope contacts to the active questionnaire organiser.
- Blind credential issuance for invited participants. New questionnaires publish per-question ballot slots; voters request a bundle of scoped blind credentials, and answer-list changes bump that question slot version so old credentials cannot be spent against the edited slot.
...
5. Voters submit public blind-token responses from ephemeral response keys, carrying either the legacy single proof or the per-question credential bundle.

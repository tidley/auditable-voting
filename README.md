...
- Coordinator **paper ballots** for voters who cannot use the digital client. The coordinator **Voters** tab has a **Paper ballots** section that builds a batch from the published questionnaire — one fresh `nsec`/`npub` identity per ballot, up to 500 in a single run — lists every generated ballot with an on-screen preview, and prints the batch through the A4 print layout. Ballots print in the selected interface language and show the questionnaire questions, the voter `nsec`/`npub` with an `npub` QR code, the invite code when one is supplied, and the relay list. Each printed ballot carries a private key, so the coordinator prints and hands it over in person; generated keys live only in the open tab, so print a batch before navigating away or refreshing. Completing the digital loop for a filled-in paper ballot is handled by the voter **paper ballot entry**.
...
1. The organiser publishes a questionnaire definition, optional non-default relay hints, and public expected-voter count. The public definition is the source of truth for organiser, voter, observer, and audit proxy. Question prompts, option labels, the title, and the description carry per-language text (`en`, `fr`, `ta`). English is required as soon as a field carries any text, because it is the fallback every reader falls back to, while the optional description may be left blank — a blank description counts as no description rather than as a missing English translation. A definition published before multi-language support stores plain English strings and is read back as English-only text, so older definitions stay usable.

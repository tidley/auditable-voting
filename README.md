...
- voters waiting for a blind credential now run a visible-page status refresh every 8 seconds, matching the manual `Refresh status` recovery path so mobile browsers surface newly delivered ballots without a tap
...
- the audit proxy can issue blind tokens (blind-signing on behalf of the coordinator for that delegated election), verify public submissions, publish public submission decisions, and (optionally) close the questionnaire after a short grace period and auto-publish a result summary once all expected invitees have accepted valid responses
- the audit proxy now polls private control-plane DMs with a seven-day fixed lookback plus event-id dedupe, so relay-randomised gift-wrap timestamps do not cause missed blind requests; status heartbeats are published back to the coordinator, runtime reads include active delegation control relays as well as startup relays, and default control relays avoid endpoints currently rejecting `#p` gift-wrap reads as unindexed
- after questionnaire publication, the coordinator Build page reveals a blue `Set up audit proxy` action that opens the separate `Audit proxy` section and generates a new audit proxy account; the section can copy a quick start command, save an autoconfigured platform-specific launcher script prefilled with the current coordinator `npub`, default helper-side logging to `RUST_LOG=debug`, refresh the helper binary from GitHub Releases on every launcher run, support right-click copy-link on those autoconfigured downloads via a safe shareable URL, and expose raw binary / direct CLI options under `Helper download and launch command`

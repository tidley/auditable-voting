...
- consume audit proxy election-config DMs carrying the blind-signing key, public questionnaire definition pointer, whitelisted voter npubs, and bounded-capacity private invite-code hashes
...
- issue blind-signature responses on behalf of the coordinator for delegated elections with `Issue blind tokens` enabled, but only after the matching election config has arrived; worker 0.1.44 or later verifies configured general-invite proof of work before eligibility mutation or signing, privately sends an authenticated ballot plan to a configured proxy voter after their first request, then holds that request until the voter returns the planned scoped request and delivers both signatures in one bundle. Unlisted general-link requests stay retryable until the organiser's later whitelist sync authorises them, and large multi-credential issuance bundles are compressed before NIP-17 encryption when that reduces the payload

...
- issue blind-signature responses on behalf of the coordinator for delegated elections with `Issue blind tokens` enabled, but only after the matching election config has arrived; worker 0.1.43 or later verifies configured general-invite proof of work before eligibility mutation or signing, unlisted general-link requests stay retryable until the organiser's later whitelist sync authorises them, and large multi-credential issuance bundles are compressed before NIP-17 encryption when that reduces the payload
